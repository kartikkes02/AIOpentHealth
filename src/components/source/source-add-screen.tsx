...
} from "@/app/api/health-data/route";
...
const personalInfoFields = (t: any, top: any): Field[] => {
  return [
    ...
    },
    {
      ...
      type: "select",
      options: [
        { value: "north_indian", label: top("ethnicity.options.northIndian") },
        { value: "south_indian", label: top("ethnicity.options.southIndian") },
        { value: "east_indian", label: top("ethnicity.options.eastIndian") },
        { value: "west_indian", label: top("ethnicity.options.westIndian") },
        {
          value: "central_indian",
          label: top("ethnicity.options.centralIndian"),
        },
        {
          value: "north_east_indian",
          label: top("ethnicity.options.northEastIndian"),
        },
      ],
    },
    {
        ...
        <Dialog open={showSettingsAlert} onOpenChange={setShowSettingsAlert}>
          <DialogContent>
            ...
            <div className="space-y-4">
              <p className="text-sm">
                Please configure the parsing settings before uploading files.
                You need to:
              </p>
              ...
              <p className="text-sm">
                You can find these settings in the Parsing Settings panel on the
                right.
              </p>
              <div className="flex justify-end">
                <Button onClick={() => setShowSettingsAlert(false)}>OK</Button>
              </div>
            </div>
          </DialogContent>
        </Dialog>
...
const HealthDataItem: React.FC<HealthDataItemProps> = ({
  ...
  const getName = (type: string) => {
    ...
    return Object.values(HealthDataType).find((t) => t.id === type)?.name || "";
  };
  ...
  isSelected
    ...
    >
      <div className="flex items-center gap-2 flex-1 min-w-0">
        <div className="flex-shrink-0">{getIcon(healthData.type)}</div>
...
const HealthDataPreview = ({
  ...
  const { ocr, dataPerPage: sourceDataPerPage } = (healthData?.metadata ||
    {}) as {
    ocr?: any;
    dataPerPage?: any;
  };
  ...
  const allInputsBlurred = Object.values(inputFocusStates).every(
    (isFocused) => !isFocused
  );
  ...
  const getNearestBoundingBox = (a: BoundingBox, b: BoundingBox): number => {
    ...
    const aDistance = Math.sqrt(
      Math.pow(aCenter.x, 2) + Math.pow(aCenter.y, 2)
    );
    const bDistance = Math.sqrt(
      Math.pow(bCenter.x, 2) + Math.pow(bCenter.y, 2)
    );

    return aDistance - bDistance;
  };

  const getFocusedWords = useCallback(
    (page: number, keyword: string): Word[] => {
      if (!keyword) return [];
      if (!ocr) return [];
      ...
      if (eFields.length === 0) {
        eFields = ocrPageData.words.filter((word) =>
          word.text.includes(keyword)
        );
      }
      return eFields.sort((a, b) =>
        getNearestBoundingBox(a.boundingBox, b.boundingBox)
      );
    },
    [ocr]
  );
  ...
  const currentPageTestResults = useMemo(() => {
    ...
    const entries = Object.entries(dataPerPage)
      .filter(([, value]) => {
  ...
  const sortedPageTestResults = useMemo(() => {
    return testItems
      .filter((item) =>
        Object.entries(currentPageTestResults).some(
          ([key, _]) => key === item.name
        )
      )
      .sort((a, b) => {
        const aFocusedWords = userBloodTestResults?.test_result[a.name]
          ? getFocusedWords(
              page,
              userBloodTestResults?.test_result[a.name].value
            )
          : [];
        const bFocusedWords = userBloodTestResults?.test_result[b.name]
          ? getFocusedWords(
              page,
              userBloodTestResults?.test_result[b.name].value
            )
          : [];
        ...
        return getNearestBoundingBox(
          aFocusedWords[0].boundingBox,
  ...
  useEffect(() => {
    ...
    if (userBloodTestResultsPage && focusedItem !== null) {
      ...
      focusedWords = Object.entries(currentPageTestResults)
        .map(([_, value]) => {
    ...
    if (focusedWords && ocr) {
      ...
      focusedWords.forEach((word) => {
        ...
        ctx.beginPath();
        ctx.moveTo(
          vertices[0].x * scaleX - paddingX,
          vertices[0].y * scaleY - paddingY
        );
        ctx.lineTo(
          vertices[1].x * scaleX + paddingX,
          vertices[1].y * scaleY - paddingY
        );
        ctx.lineTo(
          vertices[2].x * scaleX + paddingX,
          vertices[2].y * scaleY + paddingY
        );
        ctx.lineTo(
          vertices[3].x * scaleX - paddingX,
          vertices[3].y * scaleY + paddingY
        );
        ctx.closePath();
        ctx.stroke();
      });
      ...
      <div className="flex flex-col gap-4 h-full">
        <div className="h-[40%] min-h-[300px]">
          <div className="bg-white h-full overflow-y-auto rounded-lg border">
            {healthData?.type === HealthDataType.PERSONAL_INFO.id ||
            healthData?.type === HealthDataType.SYMPTOMS.id ? (
              <div className="p-4">
                <DynamicForm
                  fields={getFields()}
                  data={formData}
              ...
              healthData?.fileType?.includes("image") &&
              healthData?.filePath ? (
                <div className="p-4">
                  <Image
                ...
                <div className="bg-gray-50 p-4 rounded-lg relative flex flex-row h-full">
                  <div id="pdf" className="w-[60%] overflow-y-auto h-full">
                    ...
                    <div className="relative w-fit bottom-4 left-1/2 transform -translate-x-1/2 flex items-center space-x-4 bg-white p-2 rounded shadow">
                      <button
                  ...
                  {userBloodTestResults?.test_result && (
                    ...
                    >
                      {sortedPageTestResults.map((item) => (
                        <TextInput
                          key={item.name}
                          name={item.name.replace(/(^\w|_\w)/g, (match) =>
                            match.replace("_", "").toUpperCase()
                          )}
                          label={item.description}
                          value={
                            userBloodTestResults &&
                            userBloodTestResults.test_result
                              ? userBloodTestResults.test_result[item.name]
                                  ?.value
                              : ""
                          }
                          onChange={(v) => {
          ...
          <div className="bg-white rounded-lg border h-full flex flex-col gap-4">
            <div className="flex-1 min-h-0 p-4">
              <JSONEditor
                ...
                isEditable={
                  healthData?.type === HealthDataType.FILE.id &&
                  healthData?.status === "COMPLETED"
                }
              />
            </div>
            {healthData?.type === HealthDataType.FILE.id &&
              formData.parsingLogs && (
      ...
      {showAddFieldModal && (
        <div className="fixed top-0 left-0 w-screen h-screen bg-black bg-opacity-50 flex justify-center items-center">
          ...
          <div className="bg-white p-4 rounded-lg flex flex-col w-[50vw]">
            <p className="mb-4 font-bold">Please select a field to add</p>
            <Select
              ...
              options={testItems.map((bloodTestItem) => ({
                ...
                isDisabled: Object.entries(
                  userBloodTestResults?.test_result ?? {}
                )
                  .filter(([_, value]) => value)
                  .map(([key, _]) => key)
                  .includes(bloodTestItem.name),
              }))}
...
export default function SourceAddScreen() {
  ...
  // Vision Parser
  const [visionParser, setVisionParser] = useState<{
    value: string;
    label: string;
  }>();
  ...
  // Document Parser
  const [documentParser, setDocumentParser] = useState<{
    value: string;
    label: string;
  }>();
  const [documentParserModel, setDocumentParserModel] = useState<{
    value: string;
    label: string;
  }>();
  ...
  const { data: visionModelDataList } =
    useSWR<HealthDataParserVisionModelListResponse>(
      `/api/health-data-parser/visions/${visionParser?.value}/models?apiUrl=${visionParserApiUrl}`,
      (url: string) => fetch(url).then((res) => res.json())
    );

  const { data: documentDataList } =
    useSWR<HealthDataParserDocumentListResponse>(
      "/api/health-data-parser/documents",
      (url: string) => fetch(url).then((res) => res.json())
    );

  const { data: documentModelDataList } =
    useSWR<HealthDataParserDocumentModelListResponse>(
  ...
  const handleFileUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        // Vision Parser
        if (visionParser?.value)
          formData.append("visionParser", visionParser.value);
        if (visionParserModel?.value)
          formData.append("visionParserModel", visionParserModel.value);
        if (visionParserApiKey)
          formData.append("visionParserApiKey", visionParserApiKey);
        if (visionParserApiUrlRequired)
          formData.append("visionParserApiUrl", visionParserApiUrl);

        // Document Parser
        if (documentParser?.value)
          formData.append("documentParser", documentParser.value);
        if (documentParserModel?.value)
          formData.append("documentParserModel", documentParserModel.value);
        if (documentParserApiKey)
          formData.append("documentParserApiKey", documentParserApiKey);
        ...
        if (data.id) {
          ...
          const pollInterval = setInterval(async () => {
            try {
              ...
              const { healthData: statusData }: HealthDataGetResponse =
  ...
  const handleAddSymptoms = async (date: string) => {
    ...
    try {
      ...
      await mutate({
        healthDataList: [...(healthDataList?.healthDataList || []), newSource],
      });
    } catch (error) {
      ...
      await mutate({
        healthDataList: [...(healthDataList?.healthDataList || []), body],
      });
    }
  };
  ...
  const handleDeleteSource = async (id: string) => {
    ...
    const newSources =
      healthDataList?.healthDataList.filter((s) => s.id !== id) || [];
  ...
  const visionParserApiKeyRequired: boolean = useMemo(() => {
    return (
      visionDataList?.visions?.find((v) => v.name === visionParser?.value)
  ...
  const documentParserApiKeyRequired: boolean = useMemo(() => {
    return (
      documentDataList?.documents?.find((d) => d.name === documentParser?.value)
        ?.apiKeyRequired || false
    );
  ...
  useEffect(() => {
    if (
      visionModelDataList?.models &&
      visionModelDataList.models.length > 0 &&
      visionParserModel === undefined
    ) {
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="w-80 border-r flex flex-col">
          <div className="p-4 flex flex-col gap-4">
            <AddSourceDialog
              isSetUpVisionParser={
                visionParser !== undefined &&
                visionParserModel !== undefined &&
                (!visionParserApiKeyRequired || visionParserApiKey.length > 0)
              }
              isSetUpDocumentParser={
                documentParser !== undefined &&
                documentParserModel !== undefined &&
                (!documentParserApiKeyRequired ||
                  documentParserApiKey.length > 0)
              }
        ...
        <div className="flex-1 p-4 overflow-y-auto">
          {selectedId && healthDataList?.healthDataList && (
            <HealthDataPreview
              key={selectedId}
              healthData={
                healthDataList.healthDataList.find(
                  (s) => s.id === selectedId
                ) as HealthData
              }
              formData={formData}
                      ...
                      <div className="space-y-2">
                        <Select
                          ...
                          onChange={(selected: any) => {
                            ...
                            // Update API Url
                            const parser = visionDataList?.visions?.find(
                        ...
                        <Select
                          ...
                          onChange={(selected: any) =>
                            setVisionParserModel(selected)
                          }
                          options={visionModelDataList?.models?.map(
                            (model) => ({
                              value: model.id,
                              label: model.name,
                            })
                        ...
                        <ConditionalDeploymentEnv env={["local"]}>
                          {visionDataList?.visions?.find(
                            (v) => v.name === visionParser?.value
                          )?.apiKeyRequired && (
                            <div className="space-y-2">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setVisionParserApiKey(e.target.value)
                                }
                              />
                            </div>
                          )}
                          ...
                          {visionDataList?.visions?.find(
                            ...
                            <div className="space-y-2">
                              <input
                                ...
                                onChange={(e) =>
                                  setVisionParserApiUrl(e.target.value)
                                }
                              />
                            </div>
                          )}
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        <Select
                          ...
                          options={documentDataList?.documents?.map(
                            (document) => ({
                              value: document.name,
                              label: document.name,
                            })
                        ...
                        <Select
                          ...
                          options={documentModelDataList?.models?.map(
                            (model: any) => ({
                              value: model.id,
                              label: model.name,
                            })
                        ...
                        <ConditionalDeploymentEnv env={["local"]}>
                          {documentDataList?.documents?.find(
                            (v) => v.name === documentParser?.value
                          )?.apiKeyRequired && (
                            <div className="space-y-2">
                              <label className="text-sm font-medium">
                                {t("apiKey")}
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setDocumentParserApiKey(e.target.value)
                                }
                              />
                            </div>
                          )}
