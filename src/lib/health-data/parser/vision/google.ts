...
export class GoogleVisionParser extends BaseVisionParser {
  ...
  async parse(options: VisionParseOptions) {
    ...
    const DateNameSchema = this.removeNullable(
      z.object({
        date: HealthCheckupSchema.shape.date,
        name: HealthCheckupSchema.shape.name,
      })
    );
    const { date, name } = await messages
      .pipe(
        llm
          .withStructuredOutput(DateNameSchema, { method: "functionCalling" })
          .withConfig({
    ...
    const chunkedKeys = this.chunkArray(
      Object.keys(HealthCheckupSchema.shape.test_result.shape),
      33
    );
    ...
    const results = await processBatchWithConcurrency(
      ...
      async (chunk) => {
        return messages
          .pipe(llm.withStructuredOutput(chunk, { method: "functionCalling" }))
          .withRetry({ stopAfterAttempt: 3 })
          .invoke(options.input);
      },
      5
    ...
    return HealthCheckupSchema.parse({
      date,
      name,
      test_result: this.mergeResults(results),
    });
  }
      ...
      chunks.map(async (chunk) => {
        return z.object(
          chunk.reduce((acc, key) => {
            acc[key] = this.removeNullable(
              testResultSchema.shape[key as keyof typeof testResultSchema.shape]
            );
            return acc;
  ...
  private removeNullable<T extends ZodTypeAny>(schema: T): any {
    ...
    if (schema instanceof z.ZodArray) {
      return z.array(this.removeNullable(schema.element)) as z.ZodType<
        z.infer<T>
      >;
    }
    ...
    if (schema instanceof z.ZodOptional) {
      return z.optional(this.removeNullable(schema.unwrap())) as z.ZodType<
        z.infer<T>
      >;
    }
