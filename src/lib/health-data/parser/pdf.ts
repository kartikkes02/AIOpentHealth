...
import {
  HealthCheckupSchema,
  HealthCheckupType,
} from "@/lib/health-data/parser/schema";
import { fileTypeFromBuffer } from "file-type";
import {
  getFileMd5,
  processBatchWithConcurrency,
} from "@/lib/health-data/parser/util";
import {
  getParsePrompt,
  MessagePayload,
} from "@/lib/health-data/parser/prompt";
...
async function documentOCR({
  ...
  // Get the ocr result
  const { ocr } = await parser.ocr({
    input: document,
    model: model,
    apiKey: documentParser.apiKey,
  });
...
async function documentParse({
  ...
  const { document: result } = await parser.parse({
    input: document,
    model: model,
    apiKey: documentParser.apiKey,
  });
...
async function inference(inferenceOptions: InferenceOptions) {
  ...
  const pageDataList: { page_content: string }[] | undefined = !excludeText
    ? await processBatchWithConcurrency(
        imagePaths,
        async (path) => {
          const { content } = await documentParse({
            document: path,
            documentParser: documentParserOptions,
          });
  ...
  const batchInputs: MessagePayload[] = new Array(numPages)
    .fill(0)
    .map((_, i) => ({
      ...(!excludeText && pageDataList
        ? { context: pageDataList[i].page_content }
        : {}),
      ...(!excludeImage && imageDataList
  ...
  const messages = ChatPromptTemplate.fromMessages(
    getParsePrompt({ excludeImage, excludeText })
  );

  // Select Vision Parser
  const visionParser = visions.find(
    (e) => e.name === visionParserOptions.parser
  );
  ...
  const visionParserModel = visionParserModels.find(
    (e) => e.id === visionParserOptions.model
  );
  ...
  // Merge the results
  const data: { [key: string]: HealthCheckupType } = batchData.reduce(
    (acc, curr, i) => {
      acc[`page_${i}`] = curr;
      return acc;
    },
  ...
  for (const key of HealthCheckupSchema.shape.test_result.keyof().options) {
    ...
    for (let i = 0; i < numPages; i++) {
      ...
      if (
        healthCheckupTestResult.hasOwnProperty(key) &&
        healthCheckupTestResult[key]
      ) {
 ...
 */
async function documentToImages({
  file: filePath,
}: Pick<SourceParseOptions, "file">): Promise<string[]> {
  ...
  if (mime === "application/pdf") {
    if (currentDeploymentEnv === "local") {
      ...
      for (const image of await pdf2picConverter.bulk(-1, {
        responseType: "base64",
      })) {
  ...
  for (let i = 0; i < images.length; i++) {
    if (currentDeploymentEnv === "local") {
      fs.writeFileSync(
        `./public/uploads/${fileHash}_${i}.png`,
        Buffer.from(images[i].split(",")[1], "base64")
      );
      imagePaths.push(
        `${process.env.NEXT_PUBLIC_URL}/api/static/uploads/${fileHash}_${i}.png`
      );
    } else {
...
export async function parseHealthData(options: SourceParseOptions) {
  ...
  await processBatchWithConcurrency(
    imagePaths,
    async (path) =>
      documentParse({ document: path, documentParser: documentParser }),
    3
  );
