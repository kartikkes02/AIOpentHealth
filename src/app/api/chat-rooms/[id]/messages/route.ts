...
export interface ChatMessageListResponse { chatMessages: ChatMessage[] }

export interface ChatMessageCreateRequest {
    content: string;
    role: 'USER' | 'ASSISTANT';
    settings?: { company: string; model: string; apiEndpoint: string; apiKey: string }
}
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    ...
    const messages = [
        ...
        {
            role: "user" as const,
            content: `Health data sources: ${healthDataList.map(h => `${h.type}: ${JSON.stringify(h.data)}`).join('\n')}`
        },
        ...chatMessages.map(m => ({ role: m.role.toLowerCase() as 'user' | 'assistant', content: m.content }))
    ];
    ...
    const responseStream = new ReadableStream({
        async start(controller) {
            ...
            try {
                ...
                const gemini = new ChatGoogleGenerativeAI({ apiKey, model: llmProviderModelId })
                ...
                await prisma.$transaction(async (prisma) => {
                    await prisma.chatMessage.create({ data: { content: messageContent, role: 'ASSISTANT', chatRoomId: id } });
                    await prisma.chatRoom.update({ where: { id }, data: { lastActivityAt: new Date(), name: messageContent } });
                });
